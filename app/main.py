...
async def student_page(request: Request, code: Optional[str] = Query(None)):
    ...
    html_content = html_content.replace('Feedny', html.escape(f"Afeedny - {teacher['name']}")) # Personalize header
    html_content = html_content.replace('Afeedny', html.escape(f"Afeedny - {teacher['name']}")) # Handle rebranded name
